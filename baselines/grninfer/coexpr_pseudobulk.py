...
with torch.no_grad():
    for batch in tqdm(dm.test_dataloader(), total=len(dm.test_dataloader())):
        ...
        for sample_ix, (ixes, cnts) in enumerate(zip(batch["gene_index"], batch["gene_counts_true"])):
            ...
            obs_per_celltype[batch["0/celltype"][sample_ix]].append(batch["0/obs"][sample_ix])
...
obs_per_celltype["Myeloid cells"] = torch.stack(obs_per_celltype["Myeloid cells"])
obs_per_celltype["B cells"] = torch.stack(obs_per_celltype["B cells"])
obs_per_celltype["NK cells"] = torch.stack(obs_per_celltype["NK cells"])
obs_per_celltype["T cells"] = torch.stack(obs_per_celltype["T cells"])
...
for comb in [[1]]:
    ...
    pseudobulk_per_celltype = {}

    for celltype in obs_per_celltype:
        cntmat_ct =  countmat_per_celltype[celltype]
        obs_ct = obs_per_celltype[celltype]

        obs_ct_gr = obs_ct[:, comb]
        uniq_obs_ct = torch.unique(obs_ct_gr, dim=0, return_counts=True)[0]
        grouping_ct = torch.where((obs_ct_gr[:, None] == uniq_obs_ct[None]).all(-1))[1]
        pseudobulk_cntmat = []
        for group in torch.unique(grouping_ct):
            pseudobulk_cntmat.append(cntmat_ct[group == grouping_ct].sum(0))

        pseudobulk_cntmat = torch.stack(pseudobulk_cntmat)
        pseudobulk_cntmat = pseudobulk_cntmat / pseudobulk_cntmat.sum(1)[:, None] * 10_000
        pseudobulk_cntmat = torch.log1p(pseudobulk_cntmat)

        pseudobulk_per_celltype[celltype] = pseudobulk_cntmat
    ...
    for celltype in countmat_per_celltype:
        ...
        pseudobulk_count_mat = pseudobulk_per_celltype[celltype]

        counts_min_mean = pseudobulk_count_mat - pseudobulk_count_mat.mean(0)[None]
    ...
    print(comb, np.mean(score_per_celltype))